			...
			if(cJSON_IsString(fun)&&(fun ->valuestring != NULL ))
			{
				if(strstr(Msg ->valuestring,"fail"))                                                       //http请求通讯出错处理方法
				{
					if(request_num <= 5)
					{
						...
						{
							printf("HTTP鉴权请求失败\r\n");
							osMessageQueuePut(HTTP_REQUEST_queueHandle, &BIT_1 , 0 ,10);
						}
						...
						{
							printf("HTTP开始任务请求失败\r\n");
							osMessageQueuePut(HTTP_REQUEST_queueHandle, &BIT_2 , 0 ,10);
						}
						...
						{
							printf("HTTP继续任务失败\r\n");
							osMessageQueuePut(HTTP_REQUEST_queueHandle, &BIT_3 , 0 ,10);
						}
						...
						{
							printf("HTTP任务完成失败\r\n");
							osMessageQueuePut(HTTP_REQUEST_queueHandle, &BIT_4 , 0 ,10);
						}
						...
						{
							printf("HTTP获取分段航点失败\r\n");
							osMessageQueuePut(HTTP_REQUEST_queueHandle, &BIT_5 , 0 ,10);
						}
						...
						else if(strstr(cJSON_GetObjectItemCaseSensitive(object,"url")->valuestring,"goToCharge"))
						{
							...
							osMessageQueuePut(HTTP_REQUEST_queueHandle, &BIT_6 , 0 ,10);
						}
						request_num ++;
					}
					else
	...
	if(cJSON_IsNumber(Status)&&Status->valueint == 100)
	{
		...
		if(cJSON_IsObject(Data)&&(Data != NULL ))
		{
			...
			if( Authen_info.groupId != 0 || Authen_info.deviceId != 0)
			{
				...
				osMessageQueuePut(usart1_recv_semp_queueHandle,&null_len,0,10); //解除阻塞
				/*开启MQTT*/
